...
const socket = io.connect('https://chat-app-plod.onrender.com'); //http://localhost:3500
      ...
      <fieldset>
        ...
        >
          <TextField
            ...
            label={formErrors.room ? 'Error' : 'enter room number'}
            variant="filled"
            type="number"
            value={room}
            onChange={(e) => {
            ...
            size="small"
            helperText={formErrors.room && formErrors.room}
            color="success"
          />
          <TextField
            error={Boolean(formErrors.name)}
            label={formErrors.name ? 'Error' : 'enter your name'}
            variant="filled"
            type="text"
            value={name}
            onChange={(e) => {
        ...
        {Boolean(members) && (
          <span>
            {' '}
            Members in room :{' '}
            <button className="count" disabled={true}>
              {members}
            </button>{' '}
          </span>
        )}
          ...
          {receivedMsgs.map((ele, i) => {
            return (
              ...
              >
                ...
                <br />
                <span className="message">{ele.message}</span>
                <br />
        ...
        <div className="send-box">
          <form onSubmit={sendMessage}>
            ...
            <Button
              ...
              size="medium"
              type="submit"
              disabled={!(enableSend && room && name)}
              style={{
                marginLeft: '10px',
                boxShadow: '2px 5px 8px rgba(0,0,0,0.8)',
                marginTop: '2px',
              }}
            >
              <svg
